...
public class AdminRESTAPI {
	...
	public Response getReportCard(
			@NotNull @DecimalMin(value = "100", message = "Student ID has to be of 3 digits") @Digits(fraction = 0, integer = 3) @QueryParam("id") Integer id)
			throws ValidationException {
...
//	public void addProfessor(Professor obj, @PathParam("password") String password)
	public Response addProfessor(String str) {
		...
		if (status == 0) {
			return ResponseHelpers.badRequest(status,
					"Professor entry " + prof.getEmail() + " already exists in database.");
		}
	...
	public Response addAdmin(@Valid AddAdminHelper helper) throws ValidationException {
		...
		if (status == 0) {
			return ResponseHelpers.badRequest(status,
					"Admin entry " + admin.getEmail() + " already exists in database.");
		}
